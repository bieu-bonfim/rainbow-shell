			...
			<div class="col-md-12">
				...
				<p class='text-success text-center' id='alertas'></p>
					<div class="row no-gutters">
						...
						<input type="hidden" class="form-control" id="id-cliente" value="<?=$_SESSION['usuario']->id_usuario?>">
						...
						<div class="card-body col-md-11 mx-auto">
								...
								<table class="table table-hover table-responsive-md shopping-cart-wrap">
									<thead class="text-muted">
										<tr>
										  <th scope="col" class="text-dark" width="50">Imagem</th>
										  <th scope="col" class="text-dark">Nome</th>
										  <th scope="col" class="text-dark" width="50">Tipo</th>
										  <th scope="col" class="text-dark" width="100">Descrição</th>
										  <th scope="col" class="text-dark" width="50">Comissão</th>
										  <th scope="col" class="text-dark" width="50">Ativo</th>
										  <th scope="col" class="text-dark" width="50">Ação</th>
										</tr>
									</thead>
									<tbody>
										
										<?php if(count($estampas->listarEstampasCliente($_SESSION['usuario']->id_usuario)) > 0) : ?>	
											<?php foreach($estampas->listarEstampasCliente($_SESSION['usuario']->id_usuario) as $estampa) : ?>
											<?php $idEstampa = $estampa["id_estampa"]; ?>
											<input type="hidden" class="form-control" id="id-estampa" name="id-estampa" value="<?=$idEstampa?>">	
											<tr>
											<td>
												<figure class="media">
													<div class="img-wrap"><img src="imagens/itens/stamps/<?=$_SESSION['usuario']->id_usuario?>/<?=$estampa['imagem']?>" class="img-thumbnail"></div>														
												</figure> 
											</td>
											<td> 
											...
											<td> 
												<figcaption class="media-body">
													<h6 class="title text-truncate"><?=$estampa['tipo']?></h6>															
												</figcaption>
											</td>
											<td> 
												<figcaption class="media-body">
											...
											<td class="text-right"> 
												<a href="" class="btn btn-outline-danger" onclick="deletarEstampa(<?=$idEstampa?>)"> × Remover</a>
											</td>
											</tr>
											<?php endforeach; ?>	
										<?php else: ?>
						...
						<div class="card-body">
							<form>
								<div class="form-group">
									...
									<input id="cep_modal" type="number" pattern="[0-9]{8}" class="form-control" required>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<input id="cidade_modal" type="text" class="form-control" required>
								</div>
								<div class="form-group">
									<label for="uf">Estado*</label>
									<select id="uf_modal"  class="form-control" required>
										<option value=""></option>
				...
				<div class="modal-body">
					...
					<div class="card-body">
						<form>
							...
							<div class="form-group">
								<label>CPF:</label>
								<input type="text" class="form-control" name="cpf_modal" id="cpf_modal"  required>

							</div>

							<div class="form-group">
								<label>Telefone:</label>
								<input type="text" class="form-control" name="telefone_modal" id="telefone_modal"  required>

							</div>
										
								<div class="modal-footer">
									<button type="submit" class="btn btn-info" data-dismiss="modal" onclick="salvarDados()">Salvar Dados</button>
									<button type="button" class="btn btn-warning" data-dismiss="modal">Voltar</button>
								</div>
...
<script type="text/javascript">
		...
		$("#cep_modal").change(function(){
			...
			$.ajax({
				...
				success: function(resposta){
					...
					$("#complemento_modal").val(resposta.complemento);
	...
	function deletarEstampa(id)
	{
		...
		var idEstampa = id;
		var dt = "action=delete_estampa&idEstampa="+id;
	...
	function inserirEndereco()
	{
		...
		var cidade = $('#cidade_modal').val();
		var uf = $('#uf_modal').val();
		var idUsuario = $('#id-cliente').val();

		if(cep == "" || logradouro == "" || numero == "" || complemento == "" || bairro == "" || cidade == "" || uf == "" || idUsuario == ""){
